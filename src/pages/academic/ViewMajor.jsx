  ...
  const fetchMajor = async () => {
    try {
      const { data, error } = await supabase
        .from('majors')
        .select('*, departments(id, name_en, code), instructors:head_of_major_id(id, name_en, email), colleges(id, name_en, code)')
        .eq('id', id)
        .single()
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <p className="text-gray-900">{major?.degree_level || 'N/A'}</p>
          </div>
          <div>
          ...
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-2">Head of Major</h3>
            <p className="text-gray-900">{major?.instructors?.name_en || 'Not assigned'}</p>
          </div>
          <div>
          ...
          {major?.description && (
            <div className="md:col-span-2">
              ...
              <p className="text-gray-900">{major.description}</p>
            </div>
          )}
        </div>
      </div>
