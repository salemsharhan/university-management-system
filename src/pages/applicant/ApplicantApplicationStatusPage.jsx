...
import { FilePlus2, AlertTriangle, ChevronRight, Loader2 } from 'lucide-react'
...
/** Logged-in applicant: all applications (portal layout + sidebar). */
export default function ApplicantApplicationStatusPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto w-full min-w-0 text-start" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className={`flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8 ${isRTL ? 'sm:flex-row-reverse' : ''}`}>
        <div>
          ...
          <p className="text-sm text-[#6b7a99]">{t('applicantPortal.myApplications')}</p>
        </div>
        <Link
          to="/portal/apply"
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="rounded-xl border border-[#dde3ef] bg-white shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-[#dde3ef]">
              <h2 className="text-base font-bold text-[#1a3a6b]">{t('applicantPortal.myApplications')}</h2>
            </div>
            {rows.length === 0 ? (
